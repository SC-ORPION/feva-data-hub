...
- A receipt code to check their own choices later. Nobody else can link a ballot to a person.
...
2. Open **SQL Editor**, paste the whole of `supabase/migrations/20260924000000_voting_platform.sql` and run it. (Or, with the Supabase CLI: `supabase link` then `supabase db push`.) It removes the old data-hub tables if they exist.
...
app/
  ...
  v/[org]/…                 Voting pages: organization home, voting, public results
...
- Ranked-choice voting and automatic run-offs
